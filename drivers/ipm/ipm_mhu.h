...
/**
 * @typedef mhu_irq_config_func_t
 * @brief For configuring IRQ on each individual MHU device.
 *
 * @param mhudev Driver instance
 *
 * @internal
 */
typedef void (*mhu_irq_config_func_t)(struct device *mhudev);

struct ipm_mhu_device_config {
	u8_t *base;
	mhu_irq_config_func_t irq_config_func;
};
