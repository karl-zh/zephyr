                            ...
                            struct test_result_t *ret)
{
    ...
    if (status != PSA_SUCCESS) {
        printk("Failed to set key policy");
        return;
    }

    status = psa_get_key_information(key_handle, &type, &bits);
    if (status != PSA_ERROR_DOES_NOT_EXIST) {
        printk("Key handle does not yet contain key material");
        return;
    }
    ...
    if (status != PSA_ERROR_INVALID_HANDLE) {
        printk("Key handle should be invalid now");
        return;
    }
                     ...
                     struct test_result_t *ret)
{
    ...
    if (status != PSA_SUCCESS) {
        ...
        if (status != PSA_SUCCESS) {
            printk("Error aborting the operation");
        }
        goto destroy_key;
    }
                             ...
                             struct test_result_t *ret)
{
    ...
    status = psa_cipher_encrypt_setup(&handle, key_handle, alg);
    if (status == PSA_SUCCESS) {
        printk("Should not successfully setup an invalid cipher");
                   ...
                   struct test_result_t *ret)
{
    ...
    if (status != PSA_SUCCESS) {
        ...
        printk("Error setting up hash operation object");
        return;
    }
                  ...
                  struct test_result_t *ret)
{
    ...
    if (status != PSA_SUCCESS) {
        printk("Error setting up mac operation object");
        goto destroy_key_mac;
    }
    ...
    for (idx=0; idx<msg_num; idx++) {
        status = psa_mac_update(&handle,
                                ...
                                msg_size[idx]);
        if (status != PSA_SUCCESS) {
            printk("Error during mac operation");
            goto destroy_key_mac;
        }
    }
    ...
    if (status != PSA_SUCCESS) {
        printk("Error during finalising the mac operation");
        goto destroy_key_mac;
    }
                   ...
                   struct test_result_t *ret)
{
    ...
    if (status != PSA_SUCCESS) {
        ...
        printk("Error performing AEAD encryption");
        goto destroy_key_aead;
    }

    if (encrypted_data_length
        != PSA_AEAD_ENCRYPT_OUTPUT_SIZE(alg, sizeof(plain_text))) {
        printk("Encrypted data length is different than expected");
        goto destroy_key_aead;
    }
    ...
    if (status != PSA_SUCCESS) {
        if (status == PSA_ERROR_NOT_SUPPORTED) {
            ...
            printk("Error performing AEAD decryption");
        }
    ...
    if (sizeof(plain_text) != decrypted_data_length) {
        printk("Decrypted data length is different from plain text");
        goto destroy_key_aead;
    }
...
void psa_invalid_key_length_test(struct test_result_t *ret)
{
    ...
    if (status != PSA_ERROR_INVALID_ARGUMENT) {
        printk("Should not successfully import with an invalid key length");
        return;
    }
...
void psa_policy_key_interface_test(struct test_result_t *ret)
{
    ...
    if (usage_out != 0) {
        printk("Unexpected usage value");
        return;
    }

    alg_out = psa_key_policy_get_algorithm(&policy);
    if (alg_out != 0) {
        printk("Unexpected algorithm value");
        return;
    }
    ...
    if (status != PSA_SUCCESS) {
        printk("Failed to get key policy");
        return;
    }
    ...
    if (status != PSA_SUCCESS) {
        printk("Failed to get key lifetime");
        return;
    }

    if (lifetime_out != lifetime) {
        printk("Unexpected key lifetime value");
        return;
    }
...
void psa_policy_invalid_policy_usage_test(struct test_result_t *ret)
{
    ...
    status = psa_cipher_encrypt_setup(&handle, key_handle, alg);
    if (status != PSA_SUCCESS) {
        printk("Failed to setup cipher operation");
        goto destroy_key;
    }

    status = psa_cipher_abort(&handle);
    if (status != PSA_SUCCESS) {
        printk("Failed to abort cipher operation");
        goto destroy_key;
    }
    ...
    if (status != PSA_ERROR_NOT_PERMITTED) {
        printk("Was able to setup cipher operation with wrong alg");
        goto destroy_key;
    }
    ...
    if (status != PSA_ERROR_NOT_PERMITTED) {
        printk("Should not be able to export key without correct usage");
        goto destroy_key;
    }
