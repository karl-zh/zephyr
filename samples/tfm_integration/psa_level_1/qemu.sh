...
0x80000 -o build/tfm_qemu.bin -Binary

# Convert .bin to .elf with an appropriate offset
srec_cat build/tfm_qemu.bin -binary -offset 0x10000000 -o build/tfm_qemu.hex \
-intel --line-length=44

# Start qemu
echo "Starting QEMU (CTRL+C to quit)"
qemu-system-arm -M ${QEMUBOARD} -device loader,file=build/tfm_qemu.hex \
-serial stdio
